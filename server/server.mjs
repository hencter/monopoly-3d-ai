...
function makeNetAdapter(room) {
  ...
  return {
    ...
    async animateTeleport(p, from, to) {
      room.broadcast({ t: 'tele', player: p.id, from, to });
...
function aiEndTurnOps(room, p) {
  ...
  // 银行
  if (p.money < 160 && g.creditLimit(p) - p.debt >= 300) {
    g.borrow(p, 300);
    room.log(`${p.name} 向银行贷款 ¥300`, 'card');
  }
  if (p.debt > 0 && p.money > 800) {
    const r = g.repay(p, p.debt);
    if (r > 0) room.log(`${p.name} 偿还贷款 ¥${r}`);
  }
  // 建楼
  ...
  for (let k = 0; k < 20; k++) {
    ...
    g.buyHouse(p, c[0]);
    room.log(`${p.name} 在 ${TILES[c[0]].name} 起了一级楼`, 'good');
  }
  // 公司
  if (g.canFoundCompany(p) && p.money > 900) {
    ...
    room.log(`${p.name} 创办了 ${INDUSTRIES[ind].icon}${INDUSTRIES[ind].name} 公司！`, 'good');
  } else if (g.canUpgradeCompany(p) && p.money > 900 && g.rng() < 0.6) {
    g.upgradeCompany(p);
    room.log(`${p.name} 的公司升到 Lv${p.company.level}`, 'good');
  }
  // 拆迁卡
...
function handleOp(room, seat, msg) {
  ...
  switch (msg.op) {
    case 'build': {
      const i = msg.tileIdx;
      if (!tileOk(i) || !g.canBuild(p, i)) return err('此处不能建设');
      g.buyHouse(p, i);
      done = `${p.name} 在 ${TILES[i].name} 起了一级楼`;
      break;
    }
    case 'sellHouse': {
      const i = msg.tileIdx;
    ...
    case 'mortgage': {
      ...
      g.mortgage(p, i);
      done = `${p.name} 抵押了 ${TILES[i].name}（+¥${g.mortgageValue(i)}）`;
      break;
    }
    case 'unmortgage': {
      const i = msg.tileIdx;
    ...
    case 'foundCompany': {
      ...
      if (!g.canFoundCompany(p)) return err('现在不能创办公司');
      g.foundCompany(p, ind);
      done = `${p.name} 创办了 ${INDUSTRIES[ind].icon}${INDUSTRIES[ind].name} 公司！`;
      break;
    }
    case 'upgradeCompany': {
      if (!g.canUpgradeCompany(p)) return err('公司不能升级');
      g.upgradeCompany(p);
      done = `${p.name} 的公司升到 Lv${p.company.level}`;
      break;
    }
...
function playCard(room, p, msg, err) {
  ...
  switch (item) {
    ...
    case 'equalize': {
      ...
      return `⚖️ ${p.name} 发动均富卡，所有存活玩家现金平均为 ¥${avg}！`;
    }
    case 'rob': {
      const t = aliveTarget(msg.targetId);
      if (!t) { err('无效窃取目标'); return ''; }
      const steal = Math.min(Math.floor(t.money * 0.2), 300);
      if (steal <= 0) { err('目标没有可偷现金'); return ''; }
      g.useItem(p, 'rob');
      t.money -= steal;
      p.money += steal;
      return `🥷 ${p.name} 发动窃取卡，从 ${t.name} 处偷走 ¥${steal}！`;
    }
    case 'swap': {
...
async function runTrade(room, offer) {
  ...
  const execute = (finalPrice) => {
    ...
    room.log(`🤝 成交！${buyer.name} 以 <span class="gold">¥${finalPrice}</span> 购得 ${seller.name} 的 <b>${tileName}</b>`, 'good');
    room.update();
  };
    ...
    if (r.decision === 'accept') {
      ...
      room.pending.set(reqId, {
        ...
        resolve: (accept) => {
          ...
          else room.log(`${bySeller ? buyer.name : seller.name} 放弃了还价 ¥${r.counterPrice}`);
        },
      });
...
function onMessage(ws, msg) {
  switch (msg.t) {
    ...
    case 'join': {
      ...
      if (room.started) return err('游戏已开始，无法加入');
